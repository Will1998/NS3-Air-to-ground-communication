 ...
 */
  ...
  // bool isLoS = true;   //AtG propagation model: LoS or NLoS
   ...
   int channelWidth = 10;          
   bool guardinterval = false;      // short guard interval (sgi)
...
//    std::vector<int> Y = createArray(-200, 250, 10);
   std::vector<int> X = createArray(400, 450, 50);
   std::vector<int> Y = createArray(0, 0, 1);
   int Z = 30;
   ...
   for (uint16_t j = 0; j < dronePos_XYZ.size(); j++)
   {
      ...
      // std::string datamode = "HtMcs0";   // ErpOfdmRate54Mbps    HtMcs7
	   ...
	   */
       ...
       positionAlloc->Add (Vector (0.0, 0.0, 1)); // AP position
       ...
       if (enablePcap)
         {
    	   phy.EnablePcap ("Dji-", apDevice.Get(0));
    	   phy.EnablePcap ("Dji-", staDevice.Get(0));
         
           AsciiTraceHelper ascii;
           phy.EnableAsciiAll (ascii.CreateFileStream ("Dji.tr"));
        }
    ...
    /* Printing the Parameters Outputs */
       std::cout << std::setw (6) << j <<
        ...
        //  std::setw (14) << totalPacketsThrough;
         std::setw (15) << rxB/1000000*8/simulationTime <<
         std::setw (14) << rxPacket;
     ...
     //  if (totalPacketsThrough >= 0)
      ...
      // std::cout<< "mac rx:             "<< rxPacket << " received:   " << rxB << "  "<<rxB/1000000*8/simulationTime << " Mbit/t"<<std::endl;
    ...
    std::ofstream output_file("./scratch/Djitest.txt");
