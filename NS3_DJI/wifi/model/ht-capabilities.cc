...
void
HtCapabilities::SetMaxAmsduLength (uint32_t maxAmsduLength)
{
 // NS_ABORT_MSG_IF (maxAmsduLength != 3839 && maxAmsduLength != 7935,
  //                 "Invalid A-MSDU Max Length value");
  m_maxAmsduLength = (maxAmsduLength == 3839 ? 0 : 1);  //3839
  NS_LOG_DEBUG ("...Set Max Amsdu Length " <<m_maxAmsduLength);
}
      ...
      if ((1ul << (13 + i)) - 1 == maxAmpduLength)
        {
          ...
          NS_LOG_DEBUG ("...Set Max Ampdu Length " << maxAmpduLength << m_maxAmpduLengthExponent);
          return;
        }
    }
