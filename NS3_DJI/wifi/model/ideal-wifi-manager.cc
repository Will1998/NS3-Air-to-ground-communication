  ...
  if (GetHtSupported ())
    {
      ...
      for (uint8_t i = 0; i < nModes; i++)
        {
          for (uint16_t j = 10; j <= GetPhy ()->GetChannelWidth (); j *= 2) // j= 20
            {
              txVector.SetChannelWidth (j);
              mode = GetPhy ()->GetMcs (i);
              if (mode.GetModulationClass () == WIFI_MOD_CLASS_HT)
                {
                  uint16_t guardInterval = GetShortGuardIntervalSupported () ? 400 : 800;  // TODO
                                 ...
                                 double ctsSnr, WifiMode ctsMode, double rtsSnr)
{
  ...
  station->m_lastChannelWidthObserved = GetPhy ()->GetChannelWidth () >= 20 ? 10 : GetPhy ()->GetChannelWidth ();   // 40 ? 20
