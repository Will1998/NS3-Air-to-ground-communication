                                    ...
                                    WifiTxVector txVector, uint32_t ampduSize, Time ppduDurationLimit)
{
  ...
  if ((ppduDurationLimit.IsStrictlyPositive () && txTime > ppduDurationLimit)
    ...
    {
      NS_LOG_DEBUG("... ppduDurationLimit "<< ppduDurationLimit <<"  maxPpduDuration  "<<maxPpduDuration<<"  txTime  "<<txTime);
      NS_LOG_DEBUG ("the frame does not meet the constraint on max PPDU duration");
      return false;
    }

  NS_LOG_DEBUG("... tx time   "<< txTime);
  NS_LOG_DEBUG("... max Ppdu Duration "<< maxPpduDuration);
  NS_LOG_DEBUG("... ppdu Payload Size "<< ppduPayloadSize);
  NS_LOG_DEBUG("... maxAmpduSize "<< maxAmpduSize);
  return true;
}
              ...
              if ((*it).second.first.IsImmediateBlockAck ())
                {
                  ...
                  m_sendAckEvent = Simulator::Schedule (MicroSeconds(1),   //GetSifs (),
...
MacLow::GetBlockAckRequestDuration (WifiTxVector blockAckReqTxVector, BlockAckType type) const
{
  ...
  //NS_LOG_DEBUG ("..Block Ack Request Duration" << m_phy->CalculateTxDuration (GetBlockAckRequestSize (type), blockAckReqTxVector, m_phy->GetPhyBand ()));
}
                              ...
                              Time duration, WifiMode blockAckReqTxMode, double rxSnr)
{
  ...
  NS_ASSERT (duration.IsPositive ());
  hdr.SetDuration (MicroSeconds(1)); //duration
