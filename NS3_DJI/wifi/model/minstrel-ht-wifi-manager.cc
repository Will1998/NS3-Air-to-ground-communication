  ...
  if (GetHtSupported ())
    {
      ...
      for (uint16_t chWidth = 10; chWidth <= MAX_HT_WIDTH; chWidth *= 2)   // chwidth = 20
        {
                                                ...
                                                uint16_t chWidth, WifiMode mode, MpduType mpduType)
{
  ...
  WifiTxVector txvector;
  txvector.SetNss (streams);
  txvector.SetGuardInterval (sgi ? 12800 : 25600);   //400 : 800
  txvector.SetChannelWidth (chWidth);
  txvector.SetNess (0);
  txvector.SetStbc (0);
  txvector.SetMode (mode);
...
MinstrelHtWifiManager::DoGetDataTxVector (WifiRemoteStation *st)
{
  ...
  if (!station->m_isHt)
  ...
  else
    {
      ...
      uint64_t dataRate = mode.GetDataRate (group.chWidth, group.sgi ? 12800 : 25600, group.streams);   //400 ; 800
      if (m_currentRate != dataRate && !station->m_isSampling)
        {
          NS_LOG_DEBUG ("New datarate: " << dataRate);
          m_currentRate = dataRate;
