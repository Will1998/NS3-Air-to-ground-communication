  ...
  if (GetHtSupported ())
    {
      ...
      uint32_t maxAmsduSize = std::max ({m_voMaxAmsduSize, m_viMaxAmsduSize,
                                         m_beMaxAmsduSize, m_bkMaxAmsduSize});
      if (maxAmsduSize <= 3839)
        {
          capabilities.SetMaxAmsduLength (80000); //3839
        }
      else
        {
          capabilities.SetMaxAmsduLength (80000);   //7935
        }
      ...
      capabilities.SetMaxAmpduLength (std::min (std::max (65535u, 8191u), 65535u));
      ...
      for (uint8_t i = 0; i < m_phy->GetNMcs (); i++)
        {
          ...
          uint64_t dataRate = mcs.GetDataRate (m_phy->GetChannelWidth (), sgiSupported ? 400 : 800, nss); //TODO
