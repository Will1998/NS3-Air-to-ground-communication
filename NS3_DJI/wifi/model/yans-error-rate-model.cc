...
void
YansErrorRateModel::SetDropRate( double droprate)  
{
  m_droprate =  droprate;
  NS_LOG_DEBUG(".. set drop rate "<< m_droprate);
}
  ...
  if (mode.GetModulationClass () == WIFI_MOD_CLASS_ERP_OFDM
      ...
      || mode.GetModulationClass () == WIFI_MOD_CLASS_HE)
    {
      ...
      if(mode.GetModulationClass () == WIFI_MOD_CLASS_HT){
         ...
         NS_LOG_DEBUG("... Compare = "<< m_droprate << "," << value);
         if (value <= m_droprate){
           return 0;
           NS_LOG_DEBUG("... Drop, rate = "<< m_droprate);
         }
      }
