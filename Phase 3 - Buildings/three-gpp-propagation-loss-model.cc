...
ThreeGppPropagationLossModel::GetUtAndBsHeights (double za, double zb) const
{
  ...
  // Assuming that the tallest node is the UT (UAV) and the smallest is the BS.
  double hUt = std::max (za, zb);
  double hBs = std::min (za, zb);
...
ThreeGppUmaPropagationLossModel::GetLossLos (double distance2D, double distance3D, double hUt, double hBs) const
{
  ...
  if (hUt < 1.5 || hUt > 300)
    {
	  NS_FATAL_ERROR ("The height of the UAV should be between 1.5 and 300 m (see TR 36.777, Table B-2)");
    }

  if (hBs >= 1.5)
    {
	  NS_FATAL_ERROR ("The height of the AP should be lower than 1.5 m");
    }
  ...
  if (hUt <= 22.5)
  {
	  ...
	  // compute the breakpoint distance (see TR 36.777, Table B-2)
		double distanceBp = GetBpDistance (hUt, hBs, distance2D);
		NS_LOG_DEBUG ("breakpoint distance " << distanceBp);
...
ThreeGppUmaPropagationLossModel::GetLossNlos (double distance2D, double distance3D, double hUt, double hBs) const
{
  ...
  if (hUt <= 22.5)
  {
	  ...
	  // compute the breakpoint distance (see TR 36.777, Table B-2)
		...
		// compute the pathloss (see 3GPP TR 36.777, Table B-2)
		loss = -17.5+(46-7*log10(hUt))*log10(distance3D) + 20*log10(40*M_PI* m_frequency/(3*1e9));
  }
...
ThreeGppUmaPropagationLossModel::GetShadowingStd (Ptr<MobilityModel> a, Ptr<MobilityModel> b, ChannelCondition::LosConditionValue cond) const
{
  ...
  double hUAV = (a->GetPosition ().z > b->GetPosition ().z ? a->GetPosition ().z : b->GetPosition ().z);
  if (hUAV <= 22.5) // according to Table 7.4.1-1 of 3GPP TR 38.901
    {
	  if (cond == ChannelCondition::LosConditionValue::LOS)
		{
		  shadowingStd = 4.0;
    ...
    } else { // according to Table B-2see of 3GPP TR 36.777
  	  if (cond == ChannelCondition::LosConditionValue::LOS)
  		{
  		  shadowingStd = 4.64*exp(-0.0066*hUAV);
  		}
  	  else if (cond == ChannelCondition::LosConditionValue::NLOS)
  		{
  		  shadowingStd = 6.0;
